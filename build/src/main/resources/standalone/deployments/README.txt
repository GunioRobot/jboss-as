...
a) Deleting deployment content does not trigger undeploy. See the discussion
of the ".deployed" marker file for how to undeploy.
b) A change in any file in an exploded deployment triggers redeploy. Because
...
descriptor changes.
c) The scanner will place marker files in this directory as an indication of
...
be deployed. Instead, the scanner relies on a system of marker
...
via the "auto-deploy" attribute on the deployment-scanner element in the
...
trying to auto-deploy partially copied content.
...
E) Manual mode only: Replace currently deployed unzipped content with a new
...
F) Auto-deploy mode only: Replace currently deployed unzipped content with a
...
G) Manual mode only: Live replace portions of currently deployed unzipped
   content without redeploying:

1. cp -r target/example.war/foo.html $AS/standalone/deployments/example.war

H) Auto-deploy mode only: Live replace portions of currently deployed unzipped
   content without redeploying:

1. touch $AS/standalone/deployments/example.war.skipdeploy
2. cp -r target/example.war/foo.html $AS/standalone/deployments/example.war

I) Manual or auto-deploy mode: Redeploy currently deployed content (i.e. bounce
   it with no content change):

1. touch $AS/standalone/deployments/example.war.dodeploy

J) Auto-deploy mode only: Redeploy currently deployed content (i.e. bounce
   it with no content change):
